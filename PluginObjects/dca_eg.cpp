...
bool EnvelopeGenerator::restart()
{
	...
	if(state == egState::kAttack)
		envelopeOutput = parameters->startVal;
...
bool EnvelopeGenerator::doNoteOff(double midiPitch, uint32_t midiNoteNumber, uint32_t midiNoteVelocity)
{
	...
	if (envelopeOutput > 0 && !(parameters->egContourType == egType::kAHR_RT))
		state = egState::kRelease;
	else if (parameters->egContourType == egType::kAHR_RT)
	{
		state = egState::kFinalRelease;
	}
...
const ModOutputData EnvelopeGenerator::renderModulatorOutput()
{
	...
	switch (state)
	{
		...
		case egState::kRelease:
		{
			...
			// --- check go to next state
			if (envelopeOutput <= parameters->endVal||releaseTime_mSec <= 0.0)
			{
				envelopeOutput = 0.0;
